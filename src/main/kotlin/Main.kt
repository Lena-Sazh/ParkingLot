...
fun main() {
    ...
    while (isRunning) {
        ...
        when (input[0]) {
            ...
            "park" -> {
                if (spots != null) {
                    ...
                    if (freeSpot != -1) {
                        ...
                        spots[freeSpot].isOccupied = true
            ...
            "reg_by_color" -> {
                if (spots != null) {
                    val carPlates = mutableListOf<String>()
            ...
            "spot_by_color" -> {
                if (spots != null) {
                    val spotsNums = mutableListOf<Int>()
                    var isCarFound = false
                    for (spot in spots) {
                        if (spot.carColor.lowercase() == input[1].lowercase()) {
                            isCarFound = true
...
fun getFreeSpot(spots: MutableList<Spot>?): Int {
    if (spots != null) {
        for (i in 0..spots.size - 1) {
            if (spots[i] != null && !spots[i].isOccupied) {
                return i
            }
        }
    }
